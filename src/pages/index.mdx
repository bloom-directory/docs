...
:::warning
Bloom walletFS is alpha software. It is experimental and unaudited. Broadcast routing is enabled by default on configured chains, although policy, approval, and signing gates still apply. Start with a local network, use small amounts, and set `allow_broadcast = false` anywhere you do not intend to send.
:::
...
- [Technical architecture](/reference/technical-architecture) — crates, Machine runtime, VFS, and chain integrations.
- [Triad Architecture](/reference/triad-architecture) — Machine, Broker, Signer, custody, and approval boundaries.
