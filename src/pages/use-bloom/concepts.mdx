...
| Machine | The filesystem-facing runtime. It reads, stages, simulates, executes, and displays results, but holds no wallet keys or approval authority. |
| Broker | The policy and authorization service. It constructs reviews, hosts ceremonies, and is Machine's only authority peer. |
| Signer | The custody service. It owns wallet and delegated keys and is the only process that produces wallet-controlled signatures. |
| Sealed Approval | Bounded signing authority tied to an exact payload or tightly scoped Petal use, an exact account key, limits, policy, and expiry. It is not a custody change and does not broadcast by itself. |
| HD wallet root | The secret root material from which Signer derives wallet account keys. It and its mnemonic, seed, passphrase, and private children never enter Machine or the mount. |
| Derived account | A numbered EVM-and-Solana key family projected at `wallets/<wallet>/<number>/`. |
| Account number | The stable Signer-chosen derivation identity for a derived account, not its position in a list. Verify it against the fingerprint and path in `accounts.json` and `account.json`. |
