...
1. **Use a local network first.** Ask: "Set me up with local Anvil so nothing here involves real money, and do not stage or confirm transactions on public chains." Broadcasting is available on every configured chain, so treat policy, approval, confirmation, and Signer checks as the required safety boundaries.
