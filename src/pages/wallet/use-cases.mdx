...
BALANCE_WEI=$(cat /bloom/wallets/$WALLET/$ACCOUNT/chains/$CHAIN/balance)
...
printf 'send %s eth to %s on %s\n' "$AMOUNT_ETH" "$RECIPIENT" "$CHAIN" \
  > /bloom/wallets/$WALLET/$ACCOUNT/chains/$CHAIN/outbox/new.tx

PENDING=$(ls -t /bloom/wallets/$WALLET/$ACCOUNT/chains/$CHAIN/outbox/pending | head -n1)
cat "/bloom/wallets/$WALLET/$ACCOUNT/chains/$CHAIN/outbox/pending/$PENDING/plan.md"
cat "/bloom/wallets/$WALLET/$ACCOUNT/chains/$CHAIN/outbox/pending/$PENDING/policy_check.json" | jq .
```
...
owner=$(cat "/bloom/wallets/$WALLET/$ACCOUNT/address.evm")
