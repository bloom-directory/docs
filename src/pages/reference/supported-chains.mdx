...
Reads, simulations, and planning work with zero configuration when public RPC defaults are sufficient. Per-chain broadcast routing is enabled by default. This does not bypass policy, approval, confirmation, or Signer checks; set `allow_broadcast = false` wherever live sends are not intended.
