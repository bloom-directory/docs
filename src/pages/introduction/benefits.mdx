...
- **Multi-chain read surface.** `bloom init` configures Ethereum, Base, Tempo, Robinhood Chain, Arbitrum, Optimism, Polygon, BNB Smart Chain, Avalanche, Gnosis, Linea, HyperEVM, Arc, local Anvil, and Solana mainnet for reads.
- **Explicit account selection.** An agent acts through a numbered derived account and its authenticated public inventory rather than implicitly selecting a key.
...
- **Clear safety boundary.** Reads are broadly available; value-moving actions still require the applicable policy, approval, signing, and confirmation gates. EVM broadcast routing defaults on per configured chain; Solana mainnet requires explicit enablement plus a verified genesis pin.
